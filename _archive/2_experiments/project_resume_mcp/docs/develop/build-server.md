...
<Note>
  Servers can connect to any client. We've chosen Claude for Desktop here for simplicity, but we also have guides on [building your own client](build-client.md) as well as a [list of other clients here](/clients).
</Note>
...
1. **[Resources](../learn/server-concepts.md#resources)**: File-like data that can be read by clients (like API responses or file contents)
2. **[Tools](../learn/server-concepts.md#tools)**: Functions that can be called by the LLM (with user approval)
3. **[Prompts](../learn/server-concepts.md#prompts)**: Pre-written templates that help users accomplish specific tasks
...
<Tabs>
  <Tab title="Python">
    ...
    * You must use the Python MCP SDK 1.2.0 or higher.
    ...
    <Note>
      Claude for Desktop is not yet available on Linux. Linux users can proceed to the [Building a client](build-client.md) tutorial to build an MCP client that connects to the server we just built.
    </Note>
...
<Note>
  For more advanced troubleshooting, check out our guide on [MCP Inspector](../../legacy/tools/inspector.md)
</Note>
